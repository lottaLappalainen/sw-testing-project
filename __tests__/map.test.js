...
describe('map', () => {
  ...
  it('should handle nested arrays', () => {
    ...
    expect(map(array, iteratee)).toEqual([[2, 4], [6, 8], [10, 12]]);
  });
  ...
  it('should work with an iteratee that depends on element type', () => {
    ...
    const iteratee = (el) =>
      typeof el === 'number' ? el * 2 : typeof el === 'string' ? el.toUpperCase() : el.value;
...
it('should work with mixed types and return type-based transformations', () => {
  const array = [1, 'text', true, null, undefined];
  const iteratee = (el) => {
  ...
  expect(map(array, iteratee)).toEqual([2, 4, false, 'unknown', 'unknown']);
});
